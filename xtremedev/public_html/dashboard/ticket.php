...
<head>
    <meta charset="UTF-8">
    <title><?= ($translations['ticket'] ?? '') . " #$tid | XtremeDev" ?></title>
...
<script>
// ——— ابزار تبدیل میلادی به جلالی ساده:
...
function renderTicket(data) {
    ...
    if (data.error) {
        $loading.innerHTML = '<div class="alert alert-danger text-center">' + escapeHtml(t.not_found) + '</div>';
        return;
    }
    ...
    let statusObj = statusMap[(data.status||'').toLowerCase()] || { color: "#555", label: escapeHtml(data.status||'') };
    ...
    if (data.messages && data.messages.length) {
        data.messages.forEach(m => {
            ...
            let who = isAdmin ? t.support : t.you;
        ...
        messagesHtml = `<div style="color:#aaa;">${t.no_messages}</div>`;
    }
    ...
    if ((data.status||'').toLowerCase() !== 'closed') {
        replyFormHtml = `
            <form class="reply-form" id="reply-form" autocomplete="off">
                <label for="reply">${t.your_reply}</label>
                <textarea name="reply" id="reply" required placeholder="${t.type_reply}"></textarea>
                <button type="submit">${t.reply_btn}</button>
                <div id="reply-alert" style="margin-top:8px;"></div>
            </form>
        `;
    } else {
        replyFormHtml = `<div class="alert alert-warning text-center mb-0">${t.closed_notice}</div>`;
    }

    // دکمه بازگشت
    let backBtnHtml = `<a href="tickets.php" class="back-btn">${t.back_to_tickets}</a>`;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    .catch(err => {
        document.getElementById('ticket-loading').innerHTML =
            '<div class="alert alert-danger text-center">'+escapeHtml(t.not_found)+'</div>';
    });

    // ثبت پاسخ (reply) با API
    document.addEventListener('submit', function(ev) {
        if (ev.target && ev.target.id === 'reply-form') {
            ...
            alertBox.innerHTML = '';
            if (!reply) {
                alertBox.innerHTML = `<div class="alert alert-danger text-center">${t.msg_empty}</div>`;
                return;
            }
            if (reply.length > 2000) {
                alertBox.innerHTML = `<div class="alert alert-danger text-center">${t.msg_toolong}</div>`;
                return;
            }
            ...
            .then(async res => {
                ...
                if (res.ok && data.success) {
                    alertBox.innerHTML = `<div class="alert alert-success text-center">${t.msg_sent}</div>`;
                    setTimeout(()=>window.location.reload(), 700);
                } else {
                    alertBox.innerHTML = `<div class="alert alert-danger text-center">${t.msg_failed}</div>`;
                }
            })
            .catch(() => {
                alertBox.innerHTML = `<div class="alert alert-danger text-center">${t.msg_failed}</div>`;
