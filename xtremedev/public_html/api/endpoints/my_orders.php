...
// زبان پیش‌فرض اگر ست نشده:
$lang = trim($_GET['lang'] ?? '');
if ($lang === '') $lang = 'en';

// اگر project_id نیومده یا تهی یا صفر => همه پروژه‌ها
...
// دریافت اینویس‌ها بر اساس سفارش‌های کاربر و پروژه
$sql = "SELECT 
    i.id AS invoice_id,
    i.order_id,
    i.amount,
...
$stmt->execute();
$stmt->store_result(); // مهم: نتایج را بافر کن که بتوانی همزمان از connection استفاده کنی
$stmt->bind_result(
...
// جمع‌آوری همه product_idها
$product_ids = [];
while($stmt->fetch()) {
    $product_ids[$product_id] = true;
}
$stmt->data_seek(0); // بازگشت به ابتدای result برای استفاده دوباره از حلقه

// واکشی همه ترجمه‌ها یکجا
$translations = [];
if (!empty($product_ids)) {
    $ids = implode(',', array_keys($product_ids));
    // اول همه ترجمه‌های با زبان مورد نظر را می‌گیریم
    ...
    // سپس برای محصولات بدون ترجمه خاص، اولین ترجمه را می‌گیریم
...
// حالا از ابتدا نتایج را می‌خوانیم و ترجمه را اضافه می‌کنیم
$out = [];
$stmt->data_seek(0);
while($stmt->fetch()) {
