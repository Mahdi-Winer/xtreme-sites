...
if(!$data || !isset($data['sort_order']) || !isset($data['translations'])) {
    http_response_code(400); echo '{"error":"bad_request"}'; exit;
}
$sort_order = intval($data['sort_order']);
$stmt = $mysqli->prepare("INSERT INTO roles (sort_order) VALUES (?)");
...
foreach($data['translations'] as $lang => $t) {
    $name = $t['name'] ?? '';
    $stmt2 = $mysqli->prepare("INSERT INTO roles_translations (role_id, lang, name) VALUES (?, ?, ?)");
    $stmt2->bind_param('iss', $role_id, $lang, $name);
    $stmt2->execute();
    $stmt2->close();
}
