...
if ($can_edit && isset($_GET['delete']) && intval($_GET['delete']) > 0) {
    ...
    $api_url = 'https://api.xtremedev.co/endpoints/admin/delete_product.php';
    ...
    curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode(['product_id' => $product_id]));
    $resp = curl_exec($ch);
    $httpcode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
    curl_close($ch);

    if ($httpcode == 200) {
        $delete_msg = '<div class="alert alert-success">Product deleted successfully.</div>';
    } else {
        $delete_msg = '<div class="alert alert-danger">Delete failed or product not found.</div>';
    }
}

// --- Get products from API ---
$api_url = 'https://api.xtremedev.co/endpoints/admin/list_products.php';
$params = ['lang' => 'en'];
...
if ($httpcode === 200 && $resp) {
    ...
    if (is_array($data)) $products = $data;
} else {
    $products = [];
}
...
<head>
    <meta charset="UTF-8">
    <title>Products | XtremeDev Admin</title>
    ...
    <style>
        ...
        .btn-add-product:hover { background: #2499fa; color: #fff; }
        .footer-sticky {
            flex-shrink: 0;
            margin-top: auto;
            width: 100%;
...
<div class="container-main">
    ...
    <form method="get" class="search-box" autocomplete="off">
        <input type="text" name="search" class="search-input" placeholder="Search by ID or product name..." value="<?=htmlspecialchars($search)?>">
    ...
    <?php if($can_edit): ?>
        <a href="add_product.php" class="btn btn-add-product mb-3">Add New Product</a>
    <?php endif; ?>
        ...
        <table class="products-table">
            ...
            <?php foreach($products as $p): ?>
                <tr>
                    ...
                    <td><?=product_status_badge($p['status'])?></td>
            ...
            <?php if(empty($products)): ?>
                <tr><td colspan="<?=$can_edit?6:5?>" class="text-center text-muted" style="color:#8ba7c7;font-size:1.15rem;">No products found.</td></tr>
            <?php endif; ?>
            </tbody>
        </table>
    </div>
