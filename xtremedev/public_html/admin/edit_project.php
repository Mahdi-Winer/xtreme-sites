...
if ($project_id <= 0) {
    echo "<div style='padding:2rem;text-align:center;color:#e13a3a;font-weight:bold'>Project not found.</div>";
    exit;
}

// گرفتن اطلاعات پروژه از API (همه زبان‌های مجاز)
$translations = [];
$image = '';
$status = 'active';
$api_results = [];
foreach($allowed_langs as $lng) {
    $api_url = "https://api.xtremedev.co/endpoints/admin/get_project.php?id={$project_id}&lang={$lng}";
    ...
    $api_results[$lng] = ($httpcode === 200 && $resp) ? json_decode($resp, true) : [];
}

$project_found = false;
foreach($api_results as $lng => $data) {
    if ($data && isset($data['id'])) {
        $project_found = true;
        $translations[$lng]['title'] = $data['title'] ?? '';
        $translations[$lng]['description'] = $data['description'] ?? '';
        $translations[$lng]['long_description'] = $data['long_description'] ?? '';
        if ($image === '' && !empty($data['image'])) $image = $data['image'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validate at least title for each language
    $errors = [];
    foreach($allowed_langs as $lng) {
    ...
    if (!$errors) {
        $api_url = 'https://api.xtremedev.co/endpoints/admin/edit_project.php';
        $payload = [
            'id' => $project_id,
            'image' => $image,
            'status' => $status,
        ...
        if ($httpcode == 200) {
            ...
            $msg = '<div class="alert alert-danger">Could not update project.</div>';
        }
    } else {
...
<head>
    <meta charset="UTF-8">
    <title>Edit Project | XtremeDev Admin</title>
...
<div class="container-main">
    ...
    <div class="edit-header">
        Edit Project <span>#<?=$project_id?></span>
    </div>
