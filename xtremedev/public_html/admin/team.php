...
if ($can_edit && isset($_GET['delete']) && intval($_GET['delete']) > 0) {
    ...
    $api_url = 'https://api.xtremedev.co/endpoints/admin/team_delete.php';
    ...
    curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode(['id' => $member_id]));
    $resp = curl_exec($ch);
    $httpcode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
    curl_close($ch);

    if ($httpcode == 200) {
        $delete_msg = '<div class="alert alert-success">Member deleted successfully.</div>';
    } else {
        $delete_msg = '<div class="alert alert-danger">Delete failed or member not found.</div>';
    }
}

// Get team from API
$api_url = 'https://api.xtremedev.co/endpoints/team_list.php';
$params = ['lang' => 'en'];
...
<head>
    <meta charset="UTF-8">
    <title>Team | XtremeDev Admin</title>
    ...
    <style>
        ...
        .btn-add-member:hover { background: #2499fa; color: #fff; }
        .avatar-img { width:34px;height:34px;object-fit:cover;border-radius:50%;border:2.5px solid #31415a;background:#e4e9fa;margin-right:10px;vertical-align:middle; }
        .footer-sticky {
            flex-shrink: 0;
            margin-top: auto;
            width: 100%;
        ...
        @media (max-width: 600px) {
            ...
            .team-table-responsive {font-size:0.93rem;}
        }
    </style>
</head>
<body>
...
<div class="container-main">
    ...
    <form method="get" class="search-box" autocomplete="off">
        <input type="text" name="search" class="search-input" placeholder="Search by ID or name..." value="<?=htmlspecialchars($search)?>">
        <button type="submit" class="search-btn">Search</button>
        <?php if($search): ?>
            <a href="team.php" class="clear-btn">Clear</a>
        <?php endif; ?>
    </form>
    <?php if($can_edit): ?>
        <a href="add_team_member.php" class="btn-add-member mb-3">Add New Member</a>
    <?php endif; ?>
                ...
                <tr>
                    ...
                    <td>
                        <?php if(!empty($m['photo'])): ?>
                            <img class="avatar-img" src="<?=htmlspecialchars($m['photo'])?>" alt="">
                        <?php endif; ?>
                    </td>
                    <td><?=htmlspecialchars($m['name'])?></td>
                    <td><?=htmlspecialchars($m['role_name'])?></td>
                    <td><?=htmlspecialchars($m['skill'])?></td>
                    <td><?=htmlspecialchars($m['sub_role'])?></td>
                    <?php if($can_edit): ?>
                        <td class="action-btns">
                            <a href="edit_team_member.php?id=<?=$m['id']?>" class="btn-edit">Edit</a>
                            <a href="team.php?delete=<?=$m['id']?>" class="btn-delete" onclick="return confirm('Are you sure?')">Delete</a>
                        </td>
                    <?php endif; ?>
                </tr>
            <?php endforeach; ?>
            <?php if(empty($team)): ?>
                <tr><td colspan="<?=$can_edit?7:6?>" class="text-center text-muted" style="color:#8ba7c7;font-size:1.15rem;">No team members found.</td></tr>
            <?php endif; ?>
            </tbody>
        </table>
    </div>
