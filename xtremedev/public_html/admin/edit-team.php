...
if ($team_id) {
    $stmt = $mysqli->prepare("SELECT * FROM team WHERE id=?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !$error) {
    ...
    if (!$error) {
        $stmt = $mysqli->prepare(
            "UPDATE team SET name_en=?, name_fa=?, role_id=?, skill_en=?, skill_fa=?, priority=?, sub_role_en=?, sub_role_fa=?, long_bio_en=?, long_bio_fa=? WHERE id=?"
        );
        $stmt->bind_param(
            'ssississssi',
            $data['name_en'], $data['name_fa'], $data['role_id'], $data['skill_en'], $data['skill_fa'],
            $data['priority'], $data['sub_role_en'], $data['sub_role_fa'],
            $data['long_bio_en'], $data['long_bio_fa'], $team_id
        );
        if ($stmt->execute()) {
