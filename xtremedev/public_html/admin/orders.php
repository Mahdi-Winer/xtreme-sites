...
// --- Get orders from API ---
$api_url = 'https://api.xtremedev.co/endpoints/admin/list_orders.php';
$params = ['lang' => 'en'];
...
<head>
    <meta charset="UTF-8">
    <title>Orders | XtremeDev Admin</title>
    ...
    <style>
        ...
        .container-main { max-width: 1250px; margin:40px auto 0 auto; flex: 1 0 auto; width: 100%; }
        ...
        @media (max-width: 600px) {
            ...
            .orders-table-responsive {font-size:0.93rem;}
        }
...
<div class="container-main">
    ...
    <form method="get" class="search-box" autocomplete="off">
        <input type="text" name="search" class="search-input" placeholder="Search by user, product or status..." value="<?=htmlspecialchars($search)?>">
        <button type="submit" class="search-btn">Search</button>
        <?php if($search): ?>
            <a href="orders.php" class="clear-btn">Clear</a>
        <?php endif; ?>
    </form>
        ...
        <table class="orders-table">
            ...
            <?php foreach($orders as $ord): ?>
                <tr>
                    ...
                    <td><?=htmlspecialchars($ord['project_id'])?></td>
                    <td><?=htmlspecialchars($ord['user_name'])?></td>
                    <td><?=htmlspecialchars($ord['product_title'])?></td>
                    <td><?=status_badge($ord['status'])?></td>
                    <td><?=htmlspecialchars($ord['created_at'])?></td>
                    <td class="action-btns">
                        <?php if($can_edit): ?>
                            <a href="edit_order.php?id=<?=$ord['id']?>" class="btn-edit">Edit</a>
                        <?php else: ?>
                            <a href="order_details.php?id=<?=$ord['id']?>" class="btn-details">Details</a>
                        <?php endif; ?>
                    </td>
                </tr>
            <?php endforeach; ?>
            <?php if(empty($orders)): ?>
                <tr><td colspan="7" class="text-center text-muted" style="color:#8ba7c7;font-size:1.15rem;">No orders found.</td></tr>
            <?php endif; ?>
            </tbody>
        </table>
    </div>
