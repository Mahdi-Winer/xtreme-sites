...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if(!$project_id || !$title_fa || !$title_en) $error = 'پروژه و عنوان‌ها ضروری است.';
        ...
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS']==='on'?'https':'http')."://".$_SERVER['HTTP_HOST']."/api/endpoints/admin/articles_add.php");
        ...
        if($httpcode==200 && isset($resp['success'])) {
            ...
            $error = isset($resp['error']) ? $resp['error'] : 'خطا در ثبت مقاله';
        }
    }
}
?>
...
<div class="container-main">
    <div class="card">
        <h2 style="color:#38a8ff">افزودن مقاله</h2>
        <?php if($success): ?>
            <div class="alert alert-success">ثبت شد! <a href="articles_list.php" style="color:#fff;text-decoration:underline">بازگشت به لیست</a></div>
        <?php elseif($error): ?>
            ...
            <div class="form-group">
                ...
                <select class="form-control" name="project_id" required>
                    ...
                    <?php foreach($projectsList as $prj): ?>
                        <option value="<?=htmlspecialchars($prj['id'])?>" <?=isset($_POST['project_id'])&&$_POST['project_id']==$prj['id']?'selected':''?>><?=htmlspecialchars($prj['name'])?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="form-group">
                <label class="form-label">آدرس تصویر (اختیاری)</label>
                <input type="url" class="form-control" name="thumbnail" value="<?=isset($_POST['thumbnail'])?htmlspecialchars($_POST['thumbnail']):''?>">
            </div>
            <div class="form-group">
                <label class="form-label">عنوان (فارسی) <span style="color:#e13a3a;">*</span></label>
                <input type="text" class="form-control" name="title_fa" required value="<?=isset($_POST['title_fa'])?htmlspecialchars($_POST['title_fa']):''?>">
            </div>
            <div class="form-group">
                <label class="form-label">خلاصه (فارسی)</label>
                <input type="text" class="form-control" name="content_fa" value="<?=isset($_POST['content_fa'])?htmlspecialchars($_POST['content_fa']):''?>">
            </div>
            <div class="form-group">
                <label class="form-label">بدنه (فارسی)</label>
                <textarea class="form-control tinymce" name="body_fa"><?=isset($_POST['body_fa'])?htmlspecialchars($_POST['body_fa']):''?></textarea>
            </div>
            <div class="form-group">
                <label class="form-label">عنوان (انگلیسی) <span style="color:#e13a3a;">*</span></label>
                <input type="text" class="form-control" name="title_en" required value="<?=isset($_POST['title_en'])?htmlspecialchars($_POST['title_en']):''?>">
            </div>
            <div class="form-group">
                <label class="form-label">خلاصه (انگلیسی)</label>
                <input type="text" class="form-control" name="content_en" value="<?=isset($_POST['content_en'])?htmlspecialchars($_POST['content_en']):''?>">
            </div>
            <div class="form-group">
                <label class="form-label">بدنه (انگلیسی)</label>
                <textarea class="form-control tinymce" name="body_en"><?=isset($_POST['body_en'])?htmlspecialchars($_POST['body_en']):''?></textarea>
            </div>
            <button class="btn-main" style="margin-top:18px;">ثبت مقاله</button>
