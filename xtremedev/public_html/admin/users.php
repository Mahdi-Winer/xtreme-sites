...
$users = [];
try {
    $api_url = 'https://api.xtremedev.co/endpoints/admin/users_list.php';
    if($search !== '') $api_url .= '?search=' . urlencode($search);
...
<head>
    <meta charset="UTF-8">
    <title>Users | XtremeDev Admin</title>
    ...
    <style>
        ...
        .btn-add-user:hover { background: #2499fa; color: #fff; }
        .footer-sticky {
            flex-shrink: 0;
            margin-top: auto;
            width: 100%;
...
<div class="container-main">
    ...
    <form method="get" class="search-box" autocomplete="off">
        <input type="text" name="search" class="search-input" placeholder="Search by email, name or phone..." value="<?=htmlspecialchars($search)?>">
    ...
    <?php if($can_edit): ?>
        <a href="add_user.php" class="btn btn-add-user mb-3">Add New User</a>
    <?php endif; ?>
    ...
    <div class="users-table-responsive">
        <table class="users-table">
            ...
            <?php foreach($users as $user): ?>
                <tr>
                    <td><?=isset($user['id']) ? htmlspecialchars($user['id']) : '-'?></td>
                    <td><?=isset($user['email']) ? htmlspecialchars($user['email']) : '-'?></td>
                    <td><?=isset($user['phone']) ? htmlspecialchars($user['phone']) : '-'?></td>
                    <td><?=isset($user['fullname']) ? htmlspecialchars($user['fullname']) : '-'?></td>
                    <?php if($can_edit): ?>
            ...
            <?php endforeach; ?>
            <?php if(empty($users)): ?>
                <tr><td colspan="<?=$can_edit?5:4?>" class="text-center text-muted" style="color:#8ba7c7;font-size:1.15rem;">No users found.</td></tr>
            <?php endif; ?>
            </tbody>
        </table>
    </div>
